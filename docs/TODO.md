...
- [x] 필터 기능
  ...
  - [x] 필터 UI/UX 개선
    ...
    - [x] 필터 적용된 상태 표시
      ...
      - [x] 활성 필터 뱃지 표시 (선택 사항)
      - ---
      - [x] 필터 적용된 상태 표시 기능 구현
        - [x] Badge 컴포넌트 확인/설치
          - [x] `components/ui/badge.tsx` 파일 존재 확인
          - [x] Badge 컴포넌트 import 경로 확인
        - [x] 활성 필터 정보 타입 정의
          - [x] `ActiveFilter` 인터페이스 정의
          - [x] 필터 키, 레이블, 값, 표시 값 포함
        - [x] 활성 필터 정보 추출 함수 구현
          - [x] `getActiveFilters()` 함수 구현
          - [x] 지역 필터 정보 추출
          - [x] 관광 타입 필터 정보 추출
          - [x] 정렬 필터 정보 추출
          - [x] 반려동물 필터 정보 추출
        - [x] 활성 필터 뱃지 섹션 UI 구현
          - [x] Badge 컴포넌트 사용
          - [x] 필터 컨텐츠 상단에 뱃지 섹션 배치
          - [x] 각 필터별 뱃지 표시 (레이블: 값 형식)
          - [x] 개별 제거 버튼 (X 아이콘) 추가
          - [x] 반응형 레이아웃 (flex-wrap)
        - [x] 개별 필터 제거 핸들러 구현
          - [x] `handleRemoveFilter()` 함수 구현
          - [x] 필터 키별 제거 로직 처리
          - [x] URL 파라미터 업데이트 연동
        - [x] 필터 컨트롤 활성 상태 강조 스타일 추가
          - [x] Select 컴포넌트 강조 (지역, 정렬)
            - [x] 활성화 시 border-primary 및 bg-primary/5 적용
          - [x] Switch 컴포넌트 강조 (반려동물)
            - [x] 활성화 시 컨테이너에 border-primary 및 bg-primary/5 적용
        - [x] 데스크톱 필터 요약 정보 표시
          - [x] 필터 컨테이너 상단에 요약 텍스트 표시
          - [x] 활성 필터 개수 표시 (예: "3개 필터 적용 중")
          - [x] 모바일에서는 표시하지 않음 (토글 버튼에 이미 표시됨)
        - [x] 반응형 디자인 적용
          - [x] 모바일: 뱃지가 필터 컨텐츠 내부에 표시
          - [x] 데스크톱: 뱃지와 요약 정보 모두 표시
        - [x] 테스트 및 검증
          - [x] 필터 적용 시 뱃지 표시 확인
          - [x] 개별 필터 제거 동작 확인
          - [x] 필터 컨트롤 강조 스타일 확인
