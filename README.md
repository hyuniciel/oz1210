...
3. 또는 직접 [Supabase 통합 설정 페이지](https://dashboard.clerk.com/setup/supabase)로 이동
4. 설정 옵션을 선택한 후 **"Activate Supabase integration"** 클릭
5. 표시되는 **Clerk domain**을 복사해두세요 (예: `your-app-12.clerk.accounts.dev`)
...
4. **"Add provider"** 클릭
5. 제공자 목록에서 **"Clerk"** 선택
6. 3-1에서 복사한 **Clerk domain**을 입력 (예: `your-app-12.clerk.accounts.dev`)
...
- `auth.jwt()->>'sub'`를 통해 RLS 정책에서 Clerk user ID를 확인할 수 있습니다
- JWT 템플릿 설정이 필요하지 않습니다
...
**테스트 페이지:**
- `/instruments`: Supabase 기본 예제 (공식 문서 예제)
- `/tasks`: Clerk + Supabase 통합 예제 (RLS 정책 검증)
...
- **기본 설정**: `app/layout.tsx`의 `ClerkProvider`에서 `koKR` locale이 설정되어 있습니다.
- **커스텀 에러 메시지**: 기본 한국어 번역에 커스텀 에러 메시지가 추가되어 있습니다.
- **Tailwind CSS 4 호환성**: `cssLayerName: "clerk"` 설정으로 Tailwind CSS 4와 호환됩니다.

**로그인/회원가입 페이지:**
- `/sign-in`: 로그인 페이지 (한국어 UI)
- `/sign-up`: 회원가입 페이지 (한국어 UI)

**커스텀 로컬라이제이션 수정:**
`app/layout.tsx`의 `customKoKR` 객체를 수정하여 특정 텍스트를 커스터마이징할 수 있습니다.

자세한 내용은 [Clerk 공식 로컬라이제이션 가이드](https://clerk.com/docs/guides/customizing-clerk/localization)를 참고하세요.
...
- `authenticated` 역할은 Clerk 세션 토큰이 유효한 경우에만 부여됩니다
- 실제 예제는 `supabase/migrations/20250101000000_create_tasks_table.sql` 파일을 참고하세요
...
│   ├── tasks/            # Clerk + Supabase 통합 예제 페이지
...
│   │   └── 20250101000000_create_tasks_table.sql  # Tasks 예제 테이블
